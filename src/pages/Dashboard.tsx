...
export default function Dashboard() {
  ...
  const stats = isCoordinator
    ? [
        { label: 'Cirugías Hoy', value: mockSurgeries.length, icon: Calendar, color: 'text-primary' },
        { label: 'En Curso', value: mockSurgeries.filter((s) => !['programada', 'completada'].includes(s.status)).length, icon: Activity, color: 'text-warning' },
        { label: 'Completadas', value: mockSurgeries.filter((s) => s.status === 'completada').length, icon: CheckCircle2, color: 'text-success' },
            ...
            >
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{surgery.procedure}</p>
                  <div className="mt-3 flex flex-wrap gap-4 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1"><User className="h-3.5 w-3.5" />{surgery.team.surgeon}</span>
                  </div>
                </div>
